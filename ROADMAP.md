...
## 当前状态：✅ Phase 0/1/2/3 + 文件 I/O + 图标 + 预览可写化（含拖拽手柄）+ DataStore 主题 + 预览键盘 + 音频波形。预览屏可拖滑块/微调/拖拽手柄改时间 + 外接键盘 + 音频波形条（MediaCodec 解码降采样渲染）；设置屏切主题。剩余 Phase 4：频谱/逐帧/打轴辅助。
...
- [x] 外接键盘快捷键（预览：空格/左右 seek/Ctrl+Z 撤销）；编辑器键盘/鼠标待扩展
...
- [x] 起止滑块/微调改时间（预览可写化，共享 session）；[x] 拖拽手柄（拖行块平移改时间）
